...
private:
    struct Node {
        T value;
        //They basically provide automatic memory management
...
public:
    ...
    template <class ot>
    friend std::ostream& operator<<(std::ostream& os, List<T>& l); //Used to allow access to values
    // which are private to the class. Implied between objects of the same type
};
...
public:
   ...
   //How to move forward in the sequence?operator++
   //How to access an element of the sequence?operator*
    Iterator& operator++()
    {
...
template <class T>class List<T>::ConstIterator : public List<T>::Iterator
{
public:
...
template <class T>void List<T>::push_back(const T& v)
{
    Node * tmp = head.get();
    while(tmp->next)
        tmp = tmp-> next.get();
...
template <class T>std::ostream& operator<<(std::ostream& os, List<T>& l)
{
